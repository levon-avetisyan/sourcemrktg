...
import QuestionnaireForm from "../QuestionnaireForm.tsx";
import SalesRepCalendar from "../SalesRepCalendar.tsx";
...
const QuestionnaireSection = () => {
    ...
    const [responses, setResponses] = useState<{ vehicle: string | null; understood: string | null }>({
        vehicle: null,
        understood: null,
    });
        ...
        <section id="questionnaire" className="questionnaire">
            <div className="container">
                ...
                <p className="mb-4 mx-auto text-center subtitle">
                    We're looking for motivated individuals to join our team as independent sales professionals.
                    If you're goal-oriented, have a reliable vehicle, and are excited about a 1099 commission-based
                    opportunity,
                    we'd love to hear from you! Answer the quick questionnaire below to see if you're a great fit for
                    this exciting role.
                </p>
                ...
                {activeStep === 2 && (
                    <QuestionnaireForm
                        questions={[
                            ...
                            {label: 'Do you understand the position is 1099 and 100% commission?', name: 'understood'},
                        ]}
                        responses={responses}
                ...
                {activeStep === 3 && (
                    hasCompletedBooking ? <ThankYouMessage/> :
                        <SalesRepCalendar setHasCompletedBooking={setHasCompletedBooking}/>
                )}


                {activeStep !== 3 &&
                    <div className="steps-action d-flex align-items-center justify-content-center w-100 my-5">
                        <button
                            type="button"
                            onClick={handleNextStep}
                            disabled={activeStep === 1 && !hasVideoEnded}
                            className="btn btn-rounded-border btn-dark btn-lg py-3 px-5"
                        >
                            {activeStep === 1 ? 'Proceed to Questionnaire' : 'Submit'}
                        </button>
                    </div>}

                <button className="btn btn-link float-end" onClick={handleResetSteps}>reset steps</button>
            </div>
        </section>
    );
};
